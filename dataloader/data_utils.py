...
def get_supcon_base_dataloader(args, pretrain=False, session=0):
    ...
    if args.dataset == 'cub200':
        ...
        if args.rand_aug_sup_con:
            ...
            train_transform = transforms.Compose([
                transforms.Resize(256),
                transforms.RandomResizedCrop(size=384, scale=(args.min_crop_scale, 1.)),
                transforms.RandomHorizontalFlip(),
                transforms.RandomApply([
                    transforms.ColorJitter(0.4, 0.4, 0.4, 0.1)
                ], p=0.8),
    ...
    if pretrain:
        trainloader = torch.utils.data.DataLoader(dataset=trainset, batch_size=args.batch_size_pretrain, shuffle=True,
        ...
        trainloader = torch.utils.data.DataLoader(dataset=trainset, batch_size=args.batch_size_supcon_base, shuffle=True,
                                                num_workers=8, pin_memory=True, drop_last = args.drop_last_batch)
        testloader = torch.utils.data.DataLoader(
            dataset=testset, batch_size=args.batch_size_test, shuffle=False, num_workers=8, pin_memory=True)
        return trainset, trainloader, testloader
...
def get_supcon_replay_dataloader(args, session):
    txt_path = "data/index_list/" + args.dataset + "/session_" + str(session + 1) + '.txt'
    class_index = np.arange(args.base_class) # test on a small dataset for debugging purpose
    if args.dataset == 'cifar100':        
        normalize = transforms.Normalize(mean=[0.507, 0.487, 0.441], std=[0.267, 0.256, 0.276])
