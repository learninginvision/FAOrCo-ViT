...
class CIFAR10(VisionDataset):
    ...
    def __init__(self, root, train=True, transform=None, target_transform=None,
                 download=False, index=None, base_sess=None, cal_shap=False,
                 keep_all = False, base_aug_mag = 0):
        ...
        if base_sess:
            if cal_shap:
                self.data, self.targets = self.SelectfromOneClass(self.data, self.targets, index)
            else:
    ...
    def SelectfromOneClass(self, data, targets, index):
        data_tmp = []
        targets_tmp = []
        
        ind_cl = np.where(index == targets)[0]
        if type(data_tmp) == type([]):
            data_tmp = data[ind_cl]
            targets_tmp = targets[ind_cl]
        else:
