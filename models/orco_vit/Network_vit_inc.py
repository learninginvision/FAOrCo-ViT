...
from projector import Projector_delta_W, Projector_delta_W_omega, Porjector_complex, Projector

from sklearn.neighbors import KNeighborsClassifier
from sklearn.metrics import accuracy_score

from feature_aug import brightness_augmentation, brightness_augmentation_v2, brightness_augmentation_v3
# from prototype import FeatureDistribution
from prototype_v2 import FeatureDistribution
...
class PseudoTargetClassifier(nn.Module):
    def __init__(self, args, num_features):
        ...
        if self.args.cur_session == 0:
            self.classifiers = nn.Sequential(self.base_fc)
        else:
            self.classifiers = nn.Sequential(self.base_fc)
            for _ in range(self.args.cur_session):
                self.classifiers.append(nn.Linear(self.num_features, self.args.way, bias=False))
        ...
        # self.register_buffer("rv", torch.randn(self.n_inc_classes, self.num_features))
        self.register_buffer("rv", torch.randn(self.n_inc_classes - self.args.cur_session * self.args.way, self.num_features))
...
class ORCONET_ViT_Inc(nn.Module):
            ...
            for epoch in tqdm(range(self.args.pretrain_epochs), desc="Training"):
                ...
                with tqdm(enumerate(baseloader), total=len(baseloader), desc="Processing Batches") as pbar:
                    for idx, (images, label) in pbar:
                        ...
                        features = self.encode(images, aug=self.args.base_aug, base=True)
                        ...
                        if self.args.base_aug:
                            split_size = features.shape[0] // (2 * (self.args.base_num_aug)) # add original feature
                        else:
    ...
    def update_base(self, baseloader, testloader):
        # self.store_prototype(base=True, session=0, args=self.args)
